...
class Gmail(object):
    ...
    def append(self, filename):
        if self.check_appended(filename):
            log('Skipping %s - already pushed' % filename)
            return

        content = open(filename, 'rb').read()
        if content.endswith('\x00\x00\x00'):
            log('Skipping %s - corrupted' % filename)
            return

        message = email.message_from_string(content)
        timestamp = parsedate(message['date'])
        if not timestamp:
            log('Skipping %s - no date' % filename)
            return

        subject = decode_header(message['subject'])
        log('Sending %s ("%s", %d bytes)' % (filename, subject, len(content)))
        del message
        ...
        for filename in os.listdir(dirname):
            ...
            if os.path.isfile(filename):
                try:
                    ...
                    log('Unable to send %s (%s); skipping.' % (filename, ex))
                except:
                    log('Unable to send %s; dying.' % filename)
                    raise

...
(global-set-key "\C-cs"		'(lambda ()
							   (interactive)
							   (insert (format-time-string "%Y-%m-%d %H:%S Reece Hart <reecehart@gmail.com>"))))
(global-set-key "\C-c\C-s"	'(lambda ()
							   (interactive)
							   (insert (format-time-string "%Y-%m-%d %H:%S Reece Hart <reece.hart@locusdev.net>"))))
